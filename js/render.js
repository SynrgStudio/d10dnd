...
Renderer.monster = class {
	...
	static _getRenderedAbilityScores_classic ({mon}) {
		...
		const ptScoresWithDots = abvsRemaining.map(ab => {
			const score = mon[ab];
			const dots = convertScoreToDots(score);
			const dotDisplay = this._renderD10Dots(dots);
			return `<th class="ve-col-2 ve-text-center bold">${ab.toUpperCase()}<br><span class="small">${score} ${dotDisplay}</span></th>`;
		}).join("");

		return `${ptSpecial}
		<tr>${ptScoresWithDots}</tr>
